...
// Test with detailed debugging for the IDL tip
function debugIDLParsing() {
    console.log('🔍 Debugging International Driving Licence Parsing...\n');
  ...
  - id: international-driving-licence
    ...
    console.log('📄 YAML Content Analysis:');
    const lines = yamlContent.split('\n');
    
    // Find the IDL section and analyze it
    let inIDLSection = false;
    let idlLineStart = -1;
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (trimmed === '- id: international-driving-licence') {
            inIDLSection = true;
            idlLineStart = i;
            console.log(`\n🎯 Found IDL section starting at line ${i + 1}`);
            break;
        }
    }
    
    if (idlLineStart >= 0) {
        console.log('\n📋 IDL Section Analysis:');
        for (let i = idlLineStart; i < Math.min(idlLineStart + 15, lines.length); i++) {
            const line = lines[i];
            const trimmed = line.trim();
            ...
            if (trimmed === 'category: Transportation') {
                console.log('  ^^^ This is the category line that should end description mode');
                break;
            }
        }
    }
    ...
    if (idlTip) {
        ...
        // Check if the UK Government link is present
        if (idlTip.description.includes('UK Government Driving Abroad Guide')) {
            console.log('\n✅ UK Government link found in description!');
        } else {
            console.log('\n❌ UK Government link NOT found in description!');
        }
    } else {
